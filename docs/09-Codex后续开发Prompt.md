...
截至 2026-05-19，第一阶段至第八阶段已完成：

- 文档收口和远程推送已完成。
- Docker Compose 环境已通过真实启动验证，前端、后端、MySQL 和 Nginx `/api` 代理可用。
- 原“模拟支付”已升级为线下付款备注、管理员确认收款、管理员发货、用户确认收货、待支付订单取消并恢复库存的实用级订单流程。
- 商品图片上传已支持本地持久化目录、后台上传、格式和大小校验、Nginx `/uploads/` 代理访问。
- 已完成基础安全加固：真实 HTTP 错误状态、CORS 环境变量化、禁用用户 Token 拦截、管理员不能禁用自己、生产环境默认 JWT 密钥启动失败。
- 已拆分 `application.yml`、`application-dev.yml`、`application-prod.yml`，补齐 `.env.example`，并新增数据库初始化与 migration 说明。
- 管理后台已补齐低库存筛选和用户订单概要能力，管理员可完成商品、订单、用户和统计的基础运营。
- 前端已补齐 401/403/500 全局错误处理、结算页付款说明、后台关键操作确认和基础移动端适配。

后续继续开发时，请从 **第九阶段：测试和上线前验证** 开始推进；若发现已完成阶段相关回归，再按缺陷修复处理。
