...
1. 打开订单列表。
2. 查看新订单状态为“待支付”。
3. 点击付款备注，填写付款渠道、转账尾号或流水号。
4. 查看订单仍为“待支付”，等待管理员确认收款。

数据库观察：

```sql
SELECT id, order_no, status, payment_note, pay_time, ship_time, finish_time, cancel_time, confirm_time
FROM order_info
ORDER BY id DESC;
```
...
```sql
SELECT id, order_no, status, payment_note, admin_remark, shipping_no, pay_time, ship_time, confirm_time
FROM order_info
ORDER BY id DESC;
```
...
2. 普通用户登录。
3. 管理员登录。
4. 普通用户购物车、地址、下单、付款备注、确认收货。
5. 管理员确认收款、待发货订单、发货、统计。
